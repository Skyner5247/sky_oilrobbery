...
RegisterNetEvent("esx_oil_robbery:text")
AddEventHandler("esx_oil_robbery:text", function(text, time)
    ClearPrints()
...
RegisterNetEvent('esx_oil_robbery:alarmsound')
AddEventHandler('esx_oil_robbery:alarmsound', function(coords)
	PlaySoundFromCoord(soundid, "VEHICLES_HORNS_AMBULANCE_WARNING", coords.x, coords.y, coords.z)
end)

RegisterNetEvent('esx_oil_robbery:stopsound')
AddEventHandler('esx_oil_robbery:stopsound', function()
	StopSound(soundid)
end)

RegisterNetEvent('esx_oil_robbery:currentlyrobbing')
AddEventHandler('esx_oil_robbery:currentlyrobbing', function()
	robberyActive = true
end)

RegisterNetEvent('esx_oil_robbery:killblip')
AddEventHandler('esx_oil_robbery:killblip', function()
    RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_oil_robbery:setblip')
AddEventHandler('esx_oil_robbery:setblip', function(position)
...
RegisterNetEvent('esx_oil_robbery:toofarlocal')
AddEventHandler('esx_oil_robbery:toofarlocal', function()
	robberyActive = false
	esx.ShowNotification(Config.Strings["robbery_cancelled"])
end)

RegisterNetEvent('esx_oil_robbery:robberycomplete')
AddEventHandler('esx_oil_robbery:robberycomplete', function(robb)
	robberyActive = false
			...
			if dist < 10.0 then
				if not robberyActive then
					...
					if dist <= 1.0 then
						...
						if shooting then
							esx.TriggerServerCallback('esx_oil_robbery:copsonline', function(cops)
								if cops >= Config.RequiredCopsRob then
									TriggerServerEvent('esx_oil_robbery:rob', k)
								else
				...
				for q, t in pairs(miniLocations) do
					...
					if dist < 0.75 and not t.robbed then 
						...
						if pressed then
							...
							loadAnimDict(anim) 
							TaskPlayAnim(ped, anim, "pickup_low", 8.0, 1.0, -1, 2, 0, 0, 0, 0 ) 
							TriggerEvent("esx_oil_robbery:text", Config.Strings["collectinprogress"], 3000)
							DrawSubtitleTimed(2000, 1)
							Citizen.Wait(2000)
							ClearPedTasksImmediately(ped)
							TriggerServerEvent('esx_oil_robbery:recievepetrol', v)
							...
							if windowsBroken == #miniLocations then 
								t.robbed = false
								windowsBroken = 0
								TriggerServerEvent('esx_oil_robbery:endrob', k)
					...
					if robberyActive then
						if originalDist > 30.0 then
							TriggerServerEvent('esx_oil_robbery:toofar', k)
							robberyActive = false
							t.robbed = false
							windowsBroken = 0
						end
...
Citizen.CreateThread(function()
	...
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentString("Olie Verkoop")
	EndTextCommandSetBlipName(blip)
end)
		...
		if dist < 10.0 then
			...
			if not selling then
				...
				if dist <= 1.5 then
					...
					if released then
						esx.TriggerServerCallback('esx_oil_robbery:getItemAmount', function(quantity)
							if quantity >= Config.MaxpetrolSell then
								esx.TriggerServerCallback('esx_oil_robbery:copsonline', function(CopsConnected)
									if CopsConnected >= Config.RequiredCopsSell then
										selling = true
										FreezeEntityPosition(ped, true)
										TriggerEvent('esx_oil_robbery:text', Config.Strings["goldsell"], 10000)
										Citizen.Wait(3000)
										FreezeEntityPosition(ped, false)
										TriggerServerEvent('esx_oil_robbery:sellpetrol')
										selling = false
									else
